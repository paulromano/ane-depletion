...
\begin{abstract}
  ...
  of the matrix exponential is performed by using the incomplete partial fraction
  form of the Chebyshev rational approximation method.

  Simulations of a pressurized water reactor (PWR) pincell and a sodium-cooled fast reactor (SFR) assembly were carried out with OpenMC
...
time, nuclides within the material will transmute because of nuclear reactions and
...
represents a system of first-order ordinary differential equations (ODEs). In order to
...
outline these methods in OpenMC, several of which have not been
...
predicted middle-of-step composition and is used to deplete over the full step. The
...
is performed by using the fourth-order commutator-free Magnus integrator based on
\cref{eq:compose4}.
...
only conditionally stable. In many well-documented cases 
oscillatory behavior and diverging solutions have been observed. To address
this situation, \citet{dufek2013ane} proposed an implicit-Euler-like method that is
...
requirements. When an MC code is used, the expense is dominated by the time
to compute the transport solution, in other words, to evaluate $\vect{A}$ for a given
...
requires only one evaluation, and its error converges as $\mathcal{O}(h)$. The
...
solving the transport equation to estimate nuclear reaction rates but also making a
...
programming interface in the following manner. First, the user supplies a
...
determined, OpenMC will check again whether it exists in the decay sublibrary files.
...
reduce the number of nuclides in the depletion chain in order to minimize both the size
...
and reaction modes to determine what products may be produced. This process can help
...
OpenMC will set up tallies in a problem based on what transmutation reactions are
...
(n,$\alpha$). While this list is suitable for fission applications, fusion
...
actinides~\citep{haeck2012nse}. Doing so is complicated, however, by the fact that the
...
OpenMC does not currently implement a method for handling energy-dependent capture
...
spectra and are available at \url{https://openmc.org/depletion-chains}. In
the Python API, the class representing a depletion chain allows branching ratios
to be easily retrieved and changed. We note that while this treatment does not
...
recommends using linear-linear interpolation between energy points. At
the end of a transport solve, however, we typically have a one-group or multigroup
fission reaction rate---and it is not clear how this information should be used  to
...
source particle. For depletion, one must compute an absolute reaction
...
entire system (separate from the tally for transmutation reaction rates). With this
method one can account for the energy dependence of the fission
...
abstract classes defines an interface, that is, a set of methods that
...
are required in order to implement a method that takes a list of nuclide densities to be
...
required in order to implement a method that takes the burnup matrix $\vect{A}$, an
...
Subclasses of \texttt{Integrator} are required in order to implement a method that takes
...
Serpent 2.1.32 on two simple problems. The first problem is a 
PWR pincell, and the second problem is a single assembly of an
SFR. These problems were selected to cover both
...
nonzero independent yield and a zero cumulative yield in the FPY sublibrary
...
same answer. Thus, even minute differences may be indicative of a true
...
from the endf71x library directly, whereas OpenMC uses an HDF5 format that requires
...
exact same behavior as the full chain does, as one would expect. Use of the CASL chain
...
method for handling FPY energy dependence does not appear to have much
effect on $k_\text{eff}$, which is driven primarily by the prediction of
...
actinide concentrations between OpenMC and Serpent at four burnup
values. The fission products shown in \cref{fig:pwr-actinides-full}
...
tend to slightly decrease. These results correspond to exactly what is observed in
...
curve will be slightly overpredicted relative to Serpent and that fission products
away from the peak will be underpredicted. These results can be confirmed for particular
...
In this case, however, we find one clear outlier: $^{85}$Kr. Upon further
investigation, we determined that the root cause for this difference is 
that the CASL depletion chain uses a cumulative yield for $^{85}$Kr, and
...
analysis~\citep{nsc2015}. Specifically, a 2D model of a driver assembly for the 1000
...
this difference, let us look at differences in isotopic
compositions.
\begin{figure}[H]
  \centering
...
and stochastic implicit Euler integrators.
...
To assess the accuracy of the new depletion solver, we compared 
OpenMC and Serpent on a PWR pincell model and an SFR assembly model.
...
times for both the PWR and SFR models. Actinide concentrations were found
...
handling this energy dependence, so it is not possible to opine about whether one
...
results in a significantly lower runtime compared with the full depletion chain
...
Although we did observe some differences between OpenMC
and Serpent, most of the differences were related to deficiencies in the nuclear
data. As a whole, the results demonstrate that when the same nuclear data
and same methods are used in two independently developed codes, one can 
obtain results in a depletion simulation that match within 10s of pcm on
$k_\text{eff}$ and less than 1\% on isotopic concentrations.

In the present work, we have performed a cross-code comparison between
OpenMC and Serpent. While the results are certainly encouraging, we 
caution readers that no comparisons against experimental results have been
performed to date and would be needed in order to ensure the fitness for a particular
...
National Nuclear Security Administration. The submitted manuscript
was also suported in part by the U.S. Department of Energy, Office of Science, under contrct DE-AC02-06CH11357.
